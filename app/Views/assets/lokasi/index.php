                        ...
                        <div class="collapse navbar-collapse" id="navbarSupportedContent">
                            <?php if (in_groups('admin')) : ?>
                                <div class="nav-kategori">
                                    <!-- Add Product-->
                                    <a href="<?= site_url('assets/lokasi/create') ?>" class="btn btn-sm btn-dark"><span class="bi bi-plus-circle"> Tambah</span></a>
                                </div><!-- End Add Product -->
                            <?php endif; ?>
                        </div>
                    </div>
                    ...
                    <table id="datatable" class="table table-striped table-bordered dt-responsive nowrap" style="font-size:small; width:100%;">
                        ...
                        <tbody>
                            ...
                            <?php foreach ($lokasi as $p) : ?>
                                <tr>
                                    <th scope="row"><?= $i++; ?></th>
                                    <td><span class="badge text-bg-<?= ($p['kode_lokasi'] == 'A') ? 'warning' : 'danger'; ?>"><?= $p['kode_lokasi']; ?></span></td>
                                    <td><?= $p['nama_lokasi']; ?></td>
                                    <?php if (in_groups('admin')) : ?>
                                        <td>
                                            <a href="<?= site_url('assets/lokasi/edit/' . $p['id_lokasi']); ?>" class="btn btn-success btn-sm bi bi-pencil-square" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Edit"></a>
                                            <form action="<?= site_url('assets/lokasi/delete/' . $p['id_lokasi']); ?>" method="post" class="d-inline">
                                                <?= csrf_field(); ?>
